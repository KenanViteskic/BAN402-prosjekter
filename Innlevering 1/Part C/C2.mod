...
param P {K};			 #penalty for unmet demand

var x {R, F} >= 0;       #tons of milk sent from region r to facility f
var y {F, K} >= 0;       #tons of milk shipped from facility f to market k
var s {K} >= 0;			 #unmet demand in market k in tons

minimize total_costs:    #objective function, minimization of total transportation costs
	sum{r in R, f in F}x[r,f]*CX[r,f] 
	+ sum{f in F, k in K}y[f,k]*CY[f,k]
	+ sum{k in K} P[k]*s[k];
...
meet_demand {k in K}:    #sum of met and unmet demand must be equal to total demand
	sum{f in F}y[f,k] + s[k] = Z[k];