...
var p3		>= 0;		# $/t for S3´s C1

# New Objective: maximize weekly profit
maximize MaxAcceptablePrice: p3;  
...
# Profit with S3 price included:
s.t. ProfitTarget:
    sum{p in P} price[p]*y[p]
  - ( sum{s in S, c in C} cBuy[s,c]*x[s,c] + p3 * x['S3','C1'] )
  - sum{p in P} cProd[p]*y[p]
  >= 1.025 * baseProfit;

# Force S3 purchase to 1000 t/week:
s.t. S3Fix: x['S3','C1'] = 1000;
